import { arrForContainCarsColors } from "../generateProperty/generateCarsColor";
import { arrForContainCarsNames } from "../generateProperty/generateCarsName";
import { arrForCountCars, arrForCheckGarage } from "./createCars";
...
const generateCars = (): void => {
  document.body.addEventListener("click", (ev) => {
    const getCars = async () => {
      ...
      if ((<HTMLElement>ev.target).classList.contains("generate")) {
        for (let i = data.length; i < data.length + 100; i += 1) {
...
export { arrForCountCars, generateCars };
